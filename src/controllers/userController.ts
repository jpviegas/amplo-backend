...
const MOBILE_LOGIN_CODE_TTL_MS = 15 * 60 * 1000;

const generateMobileLoginCode = () => {
  const code = String(Math.floor(100000 + Math.random() * 900000));
  const codeHash = createHash("sha256").update(code).digest("hex");
  return { code, codeHash };
};

const setMobileLoginCodeData = (user: IUser, codeHash: string) => {
  user.mobileLoginCodeHash = codeHash;
  user.mobileLoginCodeExpiresAt = new Date(Date.now() + MOBILE_LOGIN_CODE_TTL_MS);
  user.mobileLoginCodeUsedAt = undefined;
};

const isMobileLoginCodeValid = (user: IUser, code: string) => {
  if (
    !user.mobileLoginCodeHash ||
    !user.mobileLoginCodeExpiresAt ||
    user.mobileLoginCodeUsedAt
  ) {
    return false;
  }
  if (user.mobileLoginCodeExpiresAt.getTime() <= Date.now()) {
    return false;
  }
  const codeHash = createHash("sha256").update(code).digest("hex");
  return codeHash === user.mobileLoginCodeHash;
};

const clearMobileLoginCodeData = (user: IUser) => {
  user.mobileLoginCodeHash = undefined;
  user.mobileLoginCodeExpiresAt = undefined;
  user.mobileLoginCodeUsedAt = new Date();
};
...
export const authUser = async (req: Request, res: Response) => {
  try {
    ...
    const isFirstMobileLogin = isMobile && !user.mobileFirstLoginCompletedAt;
    ...
    if (isFirstMobileLogin) {
      ...
      try {
        await sendTemplateEmail({
          to: user.email,
          subject: "Código de verificação para acesso pelo aplicativo",
          title: "Confirme seu primeiro acesso pelo aplicativo",
          description:
            "Detectamos que este é seu primeiro acesso pelo aplicativo. Use o código abaixo para confirmar sua identidade e cadastrar uma nova senha.",
          code,
          supportText:
            "O código expira em 15 minutos. Se você não solicitou este acesso, ignore este e-mail.",
        });
        emailSent = true;
      } catch {
        emailSent = false;
...
export const verifyMobileLoginCode = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isMobileLoginCodeValid(user, code)) {
      return res.status(400).json({
        success: false,
...
export const setMobileFirstAccessPassword = async (
  req: Request,
  res: Response,
) => {
  const userId = String(req.body?.userId || "").trim();
  ...
  if (!userId || !code || !newPassword) {
    return res.status(400).json({
      success: false,
      message: "userId, code e newPassword são obrigatórios",
    });
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      token,
      user: userResponse,
      message: "Senha cadastrada com sucesso. Login concluído.",
    });
  } catch (error: any) {
    if (error.name === "ValidationError") {
      return res.status(400).json({
    ...
    return res.status(500).json({
      success: false,
      message: "Erro ao cadastrar nova senha",
      error: error.message,
    });
  }
};
